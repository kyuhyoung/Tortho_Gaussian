...
def load_ply(path):
    ...
    opacity_rest = opacity_rest.reshape((xyz.shape[0], 1, (max_sh_degree + 1) ** 2 - 1))
    ...
    return xyz, features_dc, features_extra, opacity_dc, opacity_rest, scales, rots
...
def seamless_merge(model_path, partition_point_cloud_dir):
    ...
    for partition in partition_scene:
        ...
        xyz, features_dc, features_extra, opacity_dc, opacity_rest, scales, rots = load_ply(point_cloud_path)
        ...
        opacity_dc_list.append(opacity_dc[mask])
        opacity_rest_list.append(opacity_rest[mask])
    ...
    opacity_dc_list = np.concatenate(opacity_dc_list, axis=0)
    opacity_rest_list = np.concatenate(opacity_rest_list, axis=0)
    ...
    opacity_dc_list = opacity_dc_list[mask]
    opacity_rest_list = opacity_rest_list[mask]
    ...
    global_params = {'xyz': torch.from_numpy(points).float().cuda(),
                     ...
                     'opacity_features_dc': torch.from_numpy(opacity_dc_list).float().cuda().permute(0, 2, 1),
                     'opacity_features_rest': torch.from_numpy(opacity_rest_list).float().cuda().permute(0, 2, 1),
